...
def extract_ligand_energy_landscape(file_name: str) -> pd.DataFrame:
    ...
    interface_rmsd['interface_delta_X'] = interface_rmsd['interface_delta_X'].astype(float)
    interface_rmsd['ligand_rms_no_super_X'] = interface_rmsd['ligand_rms_no_super_X'].astype(float)
    print(type(interface_rmsd.iloc[1][1]))
